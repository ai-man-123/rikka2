...
`.trim()
 ...
 let grup = []
  ...
  async toMSG(buffer, type, opts = {}) {
   let data = await this.prepareMessage('0@s.whatsapp.net', buffer, type, opts)
...
exports.generateProfilePicture = async (buffer) => {
   ...
   var result = await cropped.scaleToFit(720, 720).getBufferAsync(jimp_1.MIME_JPEG)
    return {
        img: result,
        preview: result
    }
