...
if (!text) throw 'Teksnya?'
  if (true) {
    m.reply('*_Tunggu permintaan anda sedang diproses...._*')
      let { data } = await get('https://salism3api.pythonanywhere.com/write?text='+encodeURIComponent(text))
      let url = data.images[0]
     conn.sendMessage(m.chat, { url }, 'imageMessage', { quoted: m, caption: 'Sukses...' })  
       } else {
