  ...
  try {
    ...
    if ((isMedia && !m.message.videoMessage || isQuotedImage) && args.length == 0) {
      ...
      await ffmpeg(`./${media}`)
        ...
        .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@⁨WhatsApp⁩.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
        .toFormat('webp')
        .save(ran)
      ...
      await ffmpeg(`./${media}`)
        ...
        .on('end', async function () {
          console.log('Finish')
          buff = fs.readFileSync(ran)
          await conn.sendMessage(m.chat, buff, MessageType.sticker, { quoted: m })
          m.reply('*Support kak🙂*\nhttps://instagram.com/barxnl250_')
          fs.unlinkSync(media)
          fs.unlinkSync(ran)
        })
